...
describe('TeleportationFeature', () => {
  ...
  beforeEach(async () => {
    ...
    (FriendsStore as any)._instance = undefined;
    ...
    const request = await friendsStore.sendFriendRequest(
      player1.id,
      player1.userName,
      player2.id,
      player2.userName,
    );
    await friendsStore.acceptFriendRequest(request.id, player2.id);
  });
  ...
  describe('Cross-Town Teleportation via Socket', () => {
    beforeEach(async () => {
      ...
      await friendsStore.acceptFriendRequest(request.id, player3.id);
    });
  ...
  describe('Teleport with Friends Integration', () => {
    it('should verify players are friends before teleporting', async () => {
      ...
      const friends = friendsStore.getFriends(player1.id);
  ...
  describe('Friend Status Integration', () => {
    it('should show friend as Online when in a town', () => {
      const friends = friendsStore.getFriendsWithStatus(player1.id);
      const player2Friend = friends.find(f => f.friendId === player2.id);
    ...
    it('should maintain friend list across towns', async () => {
      ...
      const player1Friends = friendsStore.getFriends(player1.id);
      const player3Friends = friendsStore.getFriends(player3.id);
